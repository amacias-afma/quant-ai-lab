...
def lstm_fit(train_data, seq_len):
    # Data Prep
    ...
    calibrated_z = calibrated_tail(train_data, seq_len, pred_log_train)
    
    return lstm_model, calibrated_z, device
...
def calibrated_tail(train_data, seq_len, pred_log_train):
    ...
    with torch.no_grad():
        ...
        # D. Calculate Standardized Residuals (z = r / sigma)
        # We use daily volatility for normalization (vol_annual / sqrt(252))
        vol_train_daily = np.sqrt(np.exp(pred_log_train) / 10000)
        train_z_scores = train_returns_aligned.values / vol_train_daily
...
def lstm_vol_prediction(train_data, seq_len, test_data, lstm_model, device):
    ...
    # Step 3: Annualize (Sqrt to get Daily Vol -> * Sqrt(252))
    # lstm_vol_annual = np.sqrt(pred_var) * np.sqrt(252)
    lstm_vol_annual = np.sqrt(pred_var)
